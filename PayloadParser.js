function parseUplink(device, payload)
{
    ...
    if (jsonPayload.data !== null) {
        ...
        //variables STM ABAJO
        var aguarrasm = device.endpoints.byAddress(1);
        

          

        
        // Sentencia para Payload deSensor de nivel - tanque E5-D0462-Aguarras Mineral
        if (jsonPayload.pid == "E5-D0462-Aguarras")
        {
            ...
            var ultimo = "";
                for (let i = 0; i < data.length; i++) {
                    ...
                    env.log("Litros raw --> ",item.raw);
                    let aux = item.raw*1000*0.785; // Transforma de Litro a PESO
                    env.log ("Peso a GEAR-->",aux);
                    aguarrasm.updateWeightSensorStatus(aux,item.time);
